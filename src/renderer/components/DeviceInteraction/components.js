...
import IconSmoothBorders from "~/renderer/icons/SmoothBorders";
import type { ThemedComponent } from "~/renderer/styles/StyleProvider";

export const DeviceInteractionStepContainer: ThemedComponent<{}> = styled(Box).attrs(() => ({
  horizontal: true,
  ff: "Inter",
  fontSize: 3,
...
const SpinnerContainerWrapper: ThemedComponent<{ isVisible?: boolean }> = styled.div`
...
const SuccessContainerWrapper: ThemedComponent<{ isVisible?: boolean }> = styled(
  SpinnerContainerWrapper,
)`
...
const ErrorContainerWrapper: ThemedComponent<{ isVisible?: boolean }> = styled(
  SpinnerContainerWrapper,
)`
...
type ErrorDescContainerRest = {
  mt?: number,
};
type ErrorDescContainerProps = {
  ...ErrorDescContainerRest,
  error: Error,
  onRetry: void => void,
};

export const ErrorDescContainer = ({ error, onRetry, ...p }: ErrorDescContainerProps) => {
  ...
  return (
    <Box
      ...
      ff="Inter|SemiBold"
      style={{ maxWidth: 500 }}
      // $FlowFixMe WHY U NO UNDERSTAND TYPE ლ(¯ロ¯"ლ)
      {...p}
    >
